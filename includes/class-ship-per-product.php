...
if (! class_exists('Ced_ship_per_product'))
{
	...
	class Ced_ship_per_product 
	{
		...
		function __construct()
		{
			...
			add_action('wp_ajax_ced_spp_send_mail',array($this,'ced_spp_send_mail'));

		}
		
		function ced_spp_send_mail()
		{
			...
			if(isset($_POST["flag"]) && $_POST["flag"]==true && !empty($_POST["emailid"]))
			{
				...
				$message = 'This user of our woocommerce extension "Ship Per Product" wants to know more about marketplace extensions.<br>';
				$message .= 'Email of user : '.$_POST["emailid"];
				$headers = array('Content-Type: text/html; charset=UTF-8');
				$flag = wp_mail( $to, $subject, $message);	
				if($flag == 1)
				{
					echo json_encode(array('status'=>true,'msg'=>__('Soon you will receive the more details of this extension on the given mail.',"ship-per-product")));
				}
				else
				{
					echo json_encode(array('status'=>false,'msg'=>__('Sorry,an error occured.Please try again.',"ship-per-product")));
				}
			}
			else
			{
		...
		function ced_pbs_delete_rows()
		{
			...
			{
				$wpdb->delete( $table_name, array( 'unique_id' => $value) );
			}
		}
				...
				if(!empty($this->handling_fee_per_prod))
				{
					if($this->handling_cal_per_prod =='percent' && is_array($woocommerce->cart->cart_contents))
					{
						foreach ($woocommerce->cart->cart_contents as $item_id=>$contents)
						{
							...
							$surcharge += ($contents['data']->get_price() * ($this->handling_fee_per_prod / 100)) * $contents['quantity'];
						}
					}
		...
		function ced_spp_ptp_add_theme_scripts() 
		{
			...
			wp_enqueue_script( 'ced-spp-custom-admin',WP_PLUGIN_URL . '/ship-per-product/assets/js/ced_spp-custom-admin.js',array('jquery'), CED_SPPP_VER,true );
			...
			wp_localize_script('ced-spp-custom-admin','ajax_url',admin_url('admin-ajax.php'));
