...
if (! class_exists('ship_per_product'))
{
	...
	class ship_per_product extends WC_Shipping_Method 
	{
		...
		{
			if(isset($_GET["ced_spp_close"]) && $_GET["ced_spp_close"]==true)
			{
				unset($_GET["ced_spp_close"]);
				if(!session_id())
					session_start();
				$_SESSION["ced_spp_hide_email"]=true;
			}
	...
	<div class="wrap product-based-shipping-wrapper">
	<div class="wrap product-based-shipping-welcome_setting">
	...
	{
		$class_to_hide_banners = '';
		if($wp_active_sub_section=='ship-per-product-table') 
		{
			$class_to_hide_banners = 'ced_class_to_hide_banners';
			...
			$final_rate = $wpdb->get_results( "SELECT * FROM $table_name");
...
if(!session_id())
	session_start();
if(!isset($_SESSION["ced_spp_hide_email"])):
	$actual_link = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
$urlvars = parse_url($actual_link);
$url_params = $urlvars["query"];
?>
	<div class="ced_hpul_email_image <?php echo $class_to_hide_banners; ?>" >
					<div class="ced_hpul_email_main_content">
						...
						<div class=""  id="ced_hpul_loader">	
							<a target="_blank" href="https://chat.whatsapp.com/BcJ2QnysUVmB1S2wmwBSnE"><img id="ced-hpul-loading-image" src="<?php echo PLUGIN_DIR_URL.'/assets/images/market-place.jpg'?>" ></a>
						</div>
						<div class="ced_hpul_banner">
							<a target="_blank" href="https://chat.whatsapp.com/BcJ2QnysUVmB1S2wmwBSnE"><img src="<?php echo PLUGIN_DIR_URL.'/assets/images/market-place-2.jpg'?>"></a>
						</div>
...
<div class="ced_contact_menu_wrap">
	...
	<img src="<?php echo PLUGIN_DIR_URL.'assets/images/icon.png'?>" alt="" title="Click to Chat">
	</label>
...
<?php	
		...
		function ced_spp_save_per_prod_shipping() 
		{
			...
			// $post_par=	wp_get_post_parent_id($postedValues['_sku']);
					...
					if(file_exists($file)) 
					{
						...
						if($handle) 
						{
							...
							while (($data = fgetcsv($handle, 1000))!== false) 
							{
								...
								if(isset($data)&&!empty($data)&&count($data)==7){
									...
									if(isset($data[4]) && !empty($data[4])) 
									{
										...
										{	
											...
											{											
												...
												$postedValuescat[$count]['_line'] =  $data[5];
												$postedValuescat[$count]['_item'] =  $data[6];
				...
				foreach ($package['contents'] as $item_id=>$contents) 
				{
					...
					if ( $contents['quantity'] > 0 && $contents['data']->needs_shipping() ) 
					{
							...
							$country!='' ) 
						{
							...
							if($contents['data']->is_type('simple') ) 
							{
							...
							else if($contents['data']->is_type( 'variation'))
							{
